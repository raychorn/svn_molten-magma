...
if (__name__ == '__main__'):
    ...
    if (sys.platform == 'win32'):
        ...
        if (1 in run_tests):
            ...
            _xml = '<bridge username="%s" password="%s" token="%s" staging="1"><create table="Case_Watcher_List__c"><column name="Name" value="+++"/><column name="Email__c" value="rhorn@molten-magma.com"/><column name="Case_Watcher__c" value="a0t30000000CsCnAAK"/></create></bridge>' % (_username,_password,_token)
        ...
        if (2 in run_tests):
            _soql = "Select c.Case_Watcher__c, c.Email__c, c.Id, c.Name from Case_Watcher_List__c c where c.Email__c = 'rhorn@molten-magma.com'"
            _xml = '<bridge username="%s" password="%s" token="%s" staging="1"><soql>%s</soql></bridge>' % (_username,_password,_token,_soql)
            xml = rubypythonlib.salesForceConnector(_xml)
            print xml
            print '\n'
