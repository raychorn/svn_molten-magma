...
class nodeTool(Node):
    ...
    def __init__(self, username=None, upass=None, logname='sf.walkScm', sfb=None, options=None):
        ...
        if sfb is None:
            sfb = SCMTaskBranchTool(debug=self.debug, logname='sf.walkSCM')
        self.sfb  = sfb
    ...
    def processFile(self, path, fileName, action='NEW', isDir=False):
        ...
        if status == 'alert': 
            self.sfb.setLog('walkSCM is looping on a branch %s' %tokens, 'critical')
            ...
            if numCRLs < 0:
                ...
                if numCRLs < 0:
                    ...
                    if oldCR == True:
                        ...
                        self.sfb.setLog('walkSCM oldCR %s IN %s not found in SF oldCR in %s, NOT processing anymore' %(branch,scmpath,crList), 'warn')
                        return 0
                    else:
                        done = self.updateTokenFile(fileName, path, action)
                        self.sfb.setLog('walkSCM skipped %s IN %s unable to updated one of %s' %(branch,scmpath,crList), 'warn')
...
def walkSCM_updateSF(codeStream, options=None):
    ...
    ts1 = time.time()
    sfb = SCMTaskBranchTool(debug=options.debug, logname='sf.walkSCM')

    # Acquire lock - exit if script is running
    try:
        lockHandle = cronLock('sfWalkSCM.lock')
    except Exception, e:
...
def findReleasePath(stream, scmTS):
    ...
    #print " releaseDir OUTSIDE IF %s" %releaseDir
    if os.path.isdir(releaseDir):        
        ...
        for relDirPath in pathList:
            ...
            #relDatetime=relDatetime+difference1            
