...
class CustomCaseEventsSFNotify(CustomNotificationEventHandler):
    ...
    def _getLastProcessDate(self):
        checksince = getLastProcessDate({'update' : False, 'filename' : 'LastCaseWatcherProcessedDate', 'diff_minutes' : -15})
        return  checksince
    
    def updateLastProcessDate(self):        
        checksince = getLastProcessDate({'update' : True, 'filename' : 'LastCaseWatcherProcessedDate'})
        return          
    ...
    def mailFormat(self,caseSub, nid,watcherName):   
        rawEvnt=[]     
        sts = self.sfb.getServerTimestamp()
        currentSfDatetime = datetime.datetime(sts[0], sts[1], sts[2], sts[3], sts[4], sts[5], sts[6])
        emailToBeSent=self.crNotificationList(nid)                
        if caseSub is not None: 
    ...
    def getCompFlds(self, compId, crFlds,crDefFlds,cObj):                
        ...
        soql="Select CaseNumber, Id, LastModifiedDate, Status ,Component__c from Case  where Component__c='%s' and Status !='Closed' and LastModifiedDate>= %s" %(compName,windowIsoDateTime)        
    ...
    def getAccountFlds(self, acctId, crFlds,crDefFlds,cObj):                
        ...
        for cs in conts:        
            if cs not in [None,'']:       
                windowIsoDateTime=self._getLastProcessDate()       
                soql="Select CaseNumber, Id, ContactId, LastModifiedDate, Status ,Component__c from Case  where %s and Status !='Closed' and LastModifiedDate>= %s" %(cs,windowIsoDateTime)        
                ret = self.sfb.query('Case', soql=soql)
                if ret in BAD_INFO_LIST:                        
    ...
    def getUsrFlds(self, usrId, crFlds,crDefFlds,cObj):                
        ...
        temp=self.getCombinedFields(crFlds,crDefFlds)           
        windowIsoDateTime=self._getLastProcessDate()       
        soql="Select Id,CaseNumber, ContactId, OwnerId, RecordTypeId, Status,LastModifiedDate from Case  where OwnerId ='%s' and Status !='Closed' and LastModifiedDate >= %s" %(usrId,windowIsoDateTime)
        ret = self.sfb.query('Case', soql=soql)
        if ret in BAD_INFO_LIST:                        
    ...
    def getCaseComments(self,crId):
        ...
        where=[['ParentId','=',crId],'and',['CreatedDate','>=', windowIsoDateTime]]        
    ...
    def action(self):
        ...
        for actionEvent in self.actionList:
            ...
            for recipId, recipWhy in recipientMap.items():              
                ...
                if notification.get('deferred') is True:
                    sendImmediateNotification(recipId,notofication)
...
def sendImmediateNotification(self, recip, notification):
...
class CustomCaseEventsHandler(CustomCaseEventsSFNotify):
