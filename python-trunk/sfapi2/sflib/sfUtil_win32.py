...
class remote_os:
    ...
    def ssh_connect_transport(self):
        ...
        """
        if (len(self.hostname) > 0) and (len(self.username) > 0) and (len(self.password) > 0) and (self.port > 0):
            ...
            print 'WARNING :: Unable to make a connection with the server using hostname of "%s", port of "%s", username of "%s" and password of "%s".' % (hostname,port,username,password)
            return None
        return t
    ...
    def remote_os_exec_cmd(self, transport):
        """ Perform remote command execution using SSH link.
        """
        value = ''
        self.ssh = self.ssh_connect_client(self.transport)
        value = self.exec_command(ssh, self.transport)
        self.close()
        return value
...
def connectToRiver():
    return remote_os('river.moltenmagma.com', 22, 'sfscript', 'sf@magma05')
...
if __name__ == "__main__":
    ...
    if (1):
        #bash = remote_os_exec_cmd('bash --help', _connectionHandle_SSH)
        #if (len(bash) > 0):
            #lines = bash[0]._wbuffer.readlines()
            #print 'nytes[0]=[%s]' % '\n'.join(lines)
        #if (len(bash) > 1):
            #lines = bash[1]._wbuffer.readlines()
            #print 'nytes[1]=[%s]' % '\n'.join(lines)
        #if (len(bash) > 2):
            #lines = bash[2]._wbuffer.readlines()
            #print 'nytes[2]=[%s]' % '\n'.join(lines)
        bool = ros.os_path_exists('reallyfakefile_forsure.txt')
        assert bool == None, 'Oops - looks like the os_path_exists() function is broken !'
        cwd = ros.os_getcwd()
        for root, dirs, files in ros.os_walk('%s/log' % cwd):
            print root, "consumes",
