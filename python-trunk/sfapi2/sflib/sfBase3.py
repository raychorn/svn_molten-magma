...
class sForceApi3(SFEntitiesMixin, SFUtilityMixin, SFTestMixin):
    ...
    def loginSF(self, user=None):
        ...
        self.username = "sfbatch@molten-magma.com"
        self.password = "sf@magma05"
        
        self.from_addr = 'salesforce-support@molten-magma.COM'
        self.smtp_server = '127.0.0.1'
        self.domain = 'molten-magma.com'

        import socket
        _cName = socket.gethostbyname_ex(socket.gethostname())[0]
        if (_cName == 'rhorn-lap.ad.moltenmagma.com'):
            self.username = "rhorn@molten-magma.com"
            ...
            self.password = '%s,%s' % (pp,'NHZ3awCqrcoLt1MVG4n3on9z')

        self.sfdc = None
        self.sfdc_base_url = 'https://wwww.salesforce.com'
        try:
            t = self.password.split(',')
            if (len(t) == 2):
                pwd, token = t
            else:
                pwd = t[0]
                token = None
            self.sfdc = Connection.connect(self.username, pwd, token)
            self.sfdc_base_url = self.sfdc.context.endpoint_base.replace('-api.','.')
        except ApiFault:
            exc_info = sys.exc_info()
            info_string = '\n'.join(traceback.format_exception(*exc_info))
            print >>sys.stderr, 'ERROR: Login failed for username=%s, password=%s\n%s' % (self.username,self.password,info_string)
            sys.exit(0)
        
        uid = self.getUserIdByUsername(self.username)
        self.connectuid = self.uid = uid
