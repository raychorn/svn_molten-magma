#! /usr/bin/env python
# $Header: /CVS/sfsrc/sfapi2/sflib/ZSI/TCcompound.py,v 1.1 2004/07/14 16:47:38 sfscript Exp $
'''Compound typecodes.
'''
    ...
    for o in ofwhat:
        if not isinstance(o, TypeCode):
            raise TypeError(
                ...
                str(o.__class__))
        if o.pname is None:
            msg = tcname + ' element ' + str(o) + ' has no name'
            #print msg
...
class Struct(TypeCode):
    ...
    mutable=1, hasextras=0, **kw):
        ...
        t = type(ofwhat)
        #print 'Init of %s type:%s in %s \nofwhat:%s\n' %(pyclass, self.type, self, ofwhat)
    ...
    def serialize(self, sw, pyobj, inline=None, name=None, attrtext='', **kw):
        n = name or self.oname
        #print 'Serialize.Compound %s of \n\to:%s a:%s \n\tofwhat:%s'%(n, self.aname, self.oname, self.ofwhat)
    ...
    def cb(self, sw, pyobj, name=None, **kw):
        n = name or self.oname
        #print 'In CB %s ' %(self.pyclass)
        if not self.mutable and sw.Known(pyobj): return
        objid = '%x' % long(id(pyobj))
        n = name or self.oname or ('E' + objid)
        if self.inline:
            print >>sw, '<%s>' % n
        else:
        ...
        for what in self.ofwhat:
            ...
            if v is None:
                #print 'NO Value for %s found in %s, skipping'%(what.aname, what)
                continue
            try:
                if what.repeatable and type(v) in _seqtypes:
                    for v2 in v: what.serialize(sw, v2)
                else:
                    if hasattr(v, 'oname'): i = v.oname
                    else: i = v
                    #print '\nTrying to serialize %s in %s' %(i,n)
                    what.serialize(sw, v)
            except Exception, e:
