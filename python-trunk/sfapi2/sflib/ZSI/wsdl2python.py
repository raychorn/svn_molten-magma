...
# $Id: wsdl2python.py 25 2006-05-24 18:12:14Z misha $
    ...
    class MessageWriter:
        ...
        def fromMessageEncoded(self, message, name, namespace, style, op):
            ...
            self.typecode += '\n%sdef __init__( self, name=None, ns=None, **kw ):'\
                             % ( ID1 )
            self.typecode += "\n%s%s.__init__( self, name=%s, ns='%s', **kw )"\
        ...
        def fromMessageLiteral(self, message, name, namespace, style):
            ...
            for p in message.getPartList():
                if p.getElement():
                    ...
                    if tp.name:
                        ...
                        self.typecode += '\n\n%sdef __init__(self, name=None, ns=None, **kw ):'\
                                         %(ID1)
                        self.typecode += '\n%s%s.%s.__init__(self, name=None, ns=None, **kw )'\
                                         % (ID2, nsp, tp.name + '_Dec')
                            # for later use in docstring generation
                        self.typeList.append(['literal', '%s.%sLOCAL_Def' % (nsp, tp.name), False])
                    else:
                        self.typecode = '\n\nclass %s: ' % (message.getName())
                        self.typecode += '\n%sdef __init__(self, name=None, ns=None, **kw ): pass' %(ID1)
                    break
                else:
                    if p.getType():
                        tp = self.__class__.PartWriter()
            ...
            self.typecode += '\n%sdef __init__( self, name=None, ns=None, **kw ):'\
                             % ID1
            self.typecode += '\n%s%s.__init__( self, name=%s, ns=None, **kw )' \
