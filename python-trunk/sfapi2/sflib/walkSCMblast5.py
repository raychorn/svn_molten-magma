...
class nodeTool(Node):
    ...
    def __init__(self, username=None, upass=None, logname='sf.walkScmBlast5', sfb=None, options=None):
        self.debug = 1
        ...
        if sfb is None:
            sfb = SCMTaskBranchTool(debug=self.debug, logname='sf.walkSCMBlast5')
        self.sfb  = sfb
    ...
    def processFile(self, path, fileName, action='NEW', isDir=False):
        ...
        if status == 'alert': 
            self.sfb.setLog('walkSCMBlast5 is looping on a branch %s' %tokens, 'critical')
            ...
            if numCRLs < 0:
                ...
                if numCRLs < 0:
                    ...
                    if oldCR == True:
                        ...
                        self.sfb.setLog('walkSCMBlast5 oldCR %s IN %s not found in SF oldCR in %s, NOT processing anymore' %(branch,scmpath,crList), 'warn')
                        return 0
                    else:
                        done = self.updateTokenFile(fileName, path, action)
                        self.sfb.setLog('walkSCMBlast5 skipped %s IN %s unable to updated one of %s' %(branch,scmpath,crList), 'warn')
...
class walkSCMdir:
    ...
    def phase0(self): # Compare everything except common subdirectories
        if (sys.platform == 'win32'):
            import sfUtil_win32
        if (os.path.exists(self.left)):
            self.left_list = _filter(os.listdir(self.left), self.hide+self.ignore)
        elif (sys.platform == 'win32'):
            self.left_list = _filter(remote_os_listdir(self.left), self.hide+self.ignore)
        else:
            self.left_list = []
        if (os.path.exists(self.right)):
            self.right_list = _filter(os.listdir(self.right), self.hide+self.ignore)
        elif (sys.platform == 'win32'):
            self.right_list = _filter(remote_os_listdir(self.right), self.hide+self.ignore)
        else:
            self.right_list = []
        self.left_list.sort()
...
def walkSCM_updateSF(codeStream, options=None):
    ...
    ts1 = time.time()
    sfb = SCMTaskBranchTool(debug=options.debug, logname='sf.walkSCMBlast5')

    # Acquire lock - exit if script is running
    try:
        lockHandle = cronLock('sfwalkSCMBlast5.lock')
    except Exception, e:
...
def main_CL():
    ...
    parser.add_option("-s", "--cs",   dest="code", default="blast5",     help="The Code Stream <4|4.0|4.1|4.2|all> to use.")
