...
def ros_join(left,right):
    if (bool_is_platform_not_windows):
        return os.path.join(left, right) 
    return ros.os_sep.join([left, right])

def ros_isdir(fpath):
    if (bool_is_platform_not_windows):
        return os.path.isdir(fpath)
    return ros.os_isdir(fpath)

def ros_listdir(fpath):
    if (bool_is_platform_not_windows):
        return os.listdir(fpath)
    return ros.os_listdir(fpath)

def ros_stat(fpath):
    if (bool_is_platform_not_windows):
        return os.stat(fpath)
    return ros.os_stat(fpath)

def ros_normpath(fpath):
    if (bool_is_platform_not_windows):
        return os.path.normpath(fpath)
    return ros.os_sep.join([p for p in fpath.split(ros.os_sep) if (len(p) > 0)])

def ros_isfile(fpath):
    if (bool_is_platform_not_windows):
        return os.path.isfile(fpath)
    return ros.os_isdir(fpath)

def ros_exists(fpath):
    if (bool_is_platform_not_windows):
        return os.path.exists(fpath)
    bool = ros.os_path_exists(fpath)
    return bool
...
def makedirs(name, mode=0777):
    ...
    """
    if (not bool_is_platform_not_windows):
        head, tail = path.split(name)
        if not tail:
            head, tail = path.split(head)
        if head and tail and not ros.os_path_exists(head):
            makedirs(head, mode)
        ros.os_mkdir(name, mode)

def ros_makedirs(fpath):
    if (bool_is_platform_not_windows):
        os.makedirs(fpath)
    else:
        makedirs(fpath)
...
def ros_walk(fpath,topdown=True,onerror=None):
    if (bool_is_platform_not_windows):
        return os.walk(fpath,topdown,onerror)
    return ros.os_walk(fpath,topdown,onerror)
...
class Node:
    ...
    def load(self, p=None):
        ...
        npath = ros_join(self.rootpath, p)
        if not ros_isdir(npath): 
            self.tool.setProblem(p, 'fileLoad', 'Not a path', npath)
            return 
        #print 'Loading %s' %npath
        f_stat = ros_stat(npath)
        self.adate = f_stat[7]
        self.mdate = f_stat[8]
        self.cdate = f_stat[9]
        self.data = ros_listdir(npath)
    ...
    def getFilesNodes(self, name=None, stream=None):
        ...
        for fname in self.data:
            id = fname.split('.')[0]
            id = self.getNid(id)
            fpath = ros_join(self.path,fname) 
...
class FileNode(Node):
    ...
    def load(self, path=None, contents=True):
        ...
        fpath = ros_normpath(ros_join(self.rootpath,path))
        if not ros_isfile(fpath): 
...
class nodeTool(Node):
    ...
    def __init__(self, username=None, upass=None, logname='sf.walkSCMTalus1.0', sfb=None, options=None):
        self.debug = 1
        ...
        if sfb is None:
            sfb = SCMTaskBranchTool(debug=self.debug, logname='sf.walkSCMTalus1.0')
        self.sfb  = sfb

        self.setupBase(username, upass)        
    ...
    def getFullPath(self, path, fileName='', root='tmp'):
        ...
        if root == 'scm':
            spath = ros_join(self.scm_root, ros.os_sep.join([path, fileName]))
        else:
            spath = ros_join(self.tmp_root, ros.os_sep.join([path, fileName]))
        return spath
    ...
    def processFile(self, path, fileName, action='NEW', isDir=False):
        ...
        if status == 'alert': 
            self.sfb.setLog('walkSCMTalus1.0 is looping on a branch %s' %tokens, 'critical')
            ...
            if numCRLs < 0:
                ...
                if numCRLs < 0:
                    ...
                    if oldCR == True:
                        ...
                        self.sfb.setLog('walkSCMTalus1.0 oldCR %s IN %s not found in SF oldCR in %s, NOT processing anymore' %(branch,scmpath,crList), 'warn')
                        return 0
                    else:
                        done = self.updateTokenFile(fileName, path, action)
                        self.sfb.setLog('walkSCMTalus1.0 skipped %s IN %s unable to updated one of %s' %(branch,scmpath,crList), 'warn')
    ...
    def updateForTeam(self, fileName, path, action ):
        ...
        if 0:
            ...
            if state == 'done':
                ...
                loadFlagPath = ros_join(self.scm_root, ros.os_sep.join([blastDir, ros.os_sep.join([checkPoint, 'LOADING'])]))
                if ros_exists(loadFlagPath):
    ...
    def updateTokenFile(self, fileName, path, action='NEW'):
        ...
        if path == '/': return 0
            ...
            if path[0] == '/': 
                path = path[1:]
        spath = ros_join(self.scm_root, ros.os_sep.join([blastDir, ros.os_sep.join([path, fileName])]))
        tpath = ros_join(self.tmp_root, ros.os_sep.join([path, fileName]))
        if ros_isdir(tpath):
            if action == 'NEW':
                try:
                    ros_mkdir(tpath)
                ...
                try:
                    ...
                    if self.debug >1:print '      with files %s' %(ros_listdir(tpath))
                except Exception, e:
                    msg = 'ERROR updateTokenFile Delete DIR Failed for %s  %s, %s\n' %(tpath, Exception, e)
                    print msg
                

        elif ros_isfile(tpath):
            if action == 'NEW':
            ...
            if action == 'NEW':
                ...
                try:
                    ...
                    if e[0] == 2:
                        print 'Making subdirectories\n'
                        ros_makedirs(ros_join(self.tmp_root, path))
                        try:
    ...
    def removeAll(self, path):
        ...
        if not ros_isdir(path):
            return
        files=ros_listdir(path)
        for x in files:
            fullpath=ros_join(path, x)
            if ros_isfile(fullpath):
                self.rmGeneric(fullpath, ros_remove)
            elif ros_isdir(fullpath):
                self.removeAll(fullpath)
                self.rmGeneric(fullpath, ros_rmdir)
...
class walkSCMdir:
    ...
    def phase0(self): # Compare everything except common subdirectories
        _left_files = ros_listdir(self.left)
        _right_files = ros_listdir(self.right)
        self.left_list = _filter(_left_files, self.hide+self.ignore)
        self.right_list = _filter(_right_files, self.hide+self.ignore)
        self.left_list.sort()
        self.right_list.sort()
        print 'BEGIN: phase0 in self.left="%s", self.right="%s"' % (self.left,self.right)
        print '\tself.left_list ::'
        for f in self.left_list:
            print '\t\t%s' % f
        print '='*80
        print ''
        print '\tself.right_list ::'
        for f in self.right_list:
            print '\t\t%s' % f
        print 'END! phase0'
        print '='*80
        print ''
    ...
    def phase1(self): # Compute common names
        ...
        print 'BEGIN: phase1 in self.left="%s", self.right="%s"' % (self.left,self.right)
        print '\tself.left_list ::'
        for f in self.left_list:
            print '\t\t%s' % f
        print '='*80
    ...
    def phase2(self): # Distinguish files, directories, funnies
        ...
        for x in self.common:
            a_path = ros_join(self.left, x)
            b_path = ros_join(self.right, x)
            ok = 1
            try:
                a_stat = ros_stat(a_path)
            except os.error, why:
                msg = 'Stat failed for %s why:%s'%(a_path,why[1])
                self.tool.sfb.dlog.error(msg)
                ok = 0
            try:
                b_stat = ros_stat(b_path)
            except os.error, why:
            ...
            if ok:
                ...
                self.common_funny.append(x)
        print 'BEGIN: phase2 in self.left="%s", self.right="%s"' % (self.left,self.right)
        print '\tself.left_list ::'
        for f in self.left_list:
            print '\t\t%s' % f
        print '='*80
    ...
    def phase3(self): # Find out differences between common files
        ...
        print 'BEGIN: phase3 in self.left="%s", self.right="%s"' % (self.left,self.right)
        print '\tself.left_list ::'
        for f in self.left_list:
            print '\t\t%s' % f
        print '='*80
    ...
    def phase4(self): # Find out differences between common subdirectories
        ...
        for x in self.common_dirs:
            a_x = ros_join(self.left, x)
            b_x = ros_join(self.right, x)
            self.subdirs[x]  = walkSCMdir(a_x, b_x, self.tool, self.ignore, self.hide, self.maxNum)
        print 'BEGIN: phase4 in self.left="%s", self.right="%s"' % (self.left,self.right)
        print '\tself.left_list ::'
        for f in self.left_list:
            print '\t\t%s' % f
        print '='*80
    ...
    def walk_breadth_dir(self, options=None): # Walks on self and subdirs recursively
        self.report(options)
        l_sd_list = [sd for sd in self.subdirs.itervalues()]
        for sd in l_sd_list:
    ...
    def report(self, options=None):
        ...
        _left_only = self.left_only
        print 'BEGIN: report in self.left="%s", self.right="%s"' % (self.left,self.right)
        ...
        if self.left_only and dname not in [None,'']:
            ...
            if procType != 'update':
                ...
                for fileName in self.left_only:
                    ...
                    if ros_exists(fpath): 
                        if ros_isdir(fpath): 
                            fileList = _filter(ros_listdir(fpath),self.hide+self.ignore)  # go down one level to process this new directory
                            sdname = ros_join(dname,fileName)                         # add this dir name to end of relative path
                            ...
                            print '(+++) LEFT-ONLY dname=%s, fileName=%s' % (dname,fileName)
        ...
        if self.right_only:
            ...
            if procType != 'update':
                ...
                for fileName in self.right_only:
                    ...
                    if (ros_exists(fpath)):
                        if (ros_isdir(fpath)): 
                            _files = ros_listdir(fpath)
                            fileList = _filter(_files,self.hide+self.ignore)  # go down one level to process this new directory
                            sdname = ros_join(dname,fileName)                 # add this dire name to end of relative path
                            ...
                            print '(+++) RIGHT-ONLY dname=%s, fileName=%s' % (dname,fileName)
...
def walkSCM_updateSF(codeStream, options=None):
    ...
    ts1 = time.time()
    sfb = SCMTaskBranchTool(debug=options.debug, logname='sf.walkSCMTalus1.0')

    if (bool_is_platform_not_windows):
        # Acquire lock - exit if script is running
        try:
            lockHandle = cronLock('sfwalkSCMTalus1.0.lock')
        except Exception, e:
...
def searchForBranch(branch, teamBranch=None, options=None):
    ...
    for root, dirs, files in ros_walk(tool.scm_root):  # root is just dirpath for the loop
        ...
        for file in files:
            if branchRE.search(file):
                ...
                found.append(ros_join(path, file))
                fpath = ros_join(root,file)
                if ros_isfile(fpath):
                    f_stat = ros_stat(fpath)
                    mdate = f_stat[8]
                else: mdate = 0
        ...
        for dir in dirs:
            if branchRE.search(dir):
                path = tool.getRelPath(root)
                found.append(ros_join(path, dir))
                fpath = ros_join(root,dir)
                if ros_isdir(fpath):
                    f_stat = ros_stat(fpath)
                    mdate = f_stat[8]
                else: mdate = 0
...
def resetBranch(stream, branch, options):
    ...
    for token in found:
        num += 1
        print '%s %s' %(num, token)
        tmpPath = ros_join(tool.tmp_root,token)
...
def searchForCRs(crList, stream=None):
    ...
    reports = []
    ts1 = time.time()
    for root, dirs, files in ros_walk(tool.scm_root):
        for file in files:
...
def findReleasePath(stream, scmTS):
    ...
    releaseDir = ros_join(scm_path, stream)    
    if ros_isdir(releaseDir):        
        ...
        for relDirPath in pathList:
            ...
            stat_result = ros_stat(relDirPath)
...
def findVersionNum(releasePath):
    ...
    versionTxt = None
    versionPath = ros_join(releasePath, 'version.txt')
    if ros_exists(versionPath):
...
def main_CL():
    ...
    parser.add_option("-s", "--cs",   dest="code", default="talus1.0",     help="The Code Stream <4|4.0|4.1|4.2|all> to use.")
