...
test_wx = Target(
    description = "Leads Importer",
    script = filename,
    other_resources = [(RT_MANIFEST, 1, manifest_template % dict(prog=distribution))],
    icon_resources = [(1, "salesforce2.ico")],
    dest_base = distribution)

##################################################  ##############
packages = ["lib"]
includes = ['lib.ui.PyDialog', 'wx.lib.mixins.treemixin']
...
_retry_count = 2
egg_path = _dirname = ''
_message = lambda reason:'Cannot build because "%s".' % (reason)
while (_isRunning):
    try:
        setup(
        ...
        print 'ERROR: %s.' % (_message(str(details)))
