...
class SFConfig(ConfigParser.ConfigParser):
    ...
    def __init__(self,top=None):
        ...
        self.__root__ = top
        if (sys.platform == 'win32'):
            self.ros = sfUtil_win32.connectToRiver()
    
    def __str__(self):
        import StringIO
        f = StringIO.StringIO()
        print >>f, '%s' % self.__class__
    ...
    def __read(self,fname,lines):
        import tempfile
        fd, fnameOut = tempfile.mkstemp()
        os.close(fd)
        ...
        fOut.close()
        fIn = open(fnameOut,'r')
        self._read(fIn,fname)
        fIn.close()
        if (os.path.exists(fnameOut)):
            os.remove(fnameOut)
    
    def remote_read(self,filenames):
        if isinstance(filenames, basestring):
            filenames = [filenames]
        for filename in filenames:
            _lines = self.ros.os_read_file(filename)
            self.__read(filename,_lines)
    ...
    def load(self, scope='all'):
        ...
        _sep = os.sep
        if (sys.platform == 'win32'):
            _sep = self.ros.os_sep
        if (self.root != None):
            common = self.root
        else:
            common = _sep.join(['','home','sfscript','data','sfapi'])
        dirs = [self.getHome(),common]
        if (sys.platform != 'win32'):
        ...
        for dir in dirs:
            ...
            for file in ['.sfConfig','sfConfig.ini','sForce.cfg']:
                fpath = _sep.join([dir,file])
                _isFile = False
                if (sys.platform == 'win32'):
                    _isFile = not self.ros.os_isdir(fpath)
                else:
                    _isFile = os.path.isfile(fpath)
                if (_isFile):
                    paths.append(fpath)
        if (sys.platform == 'win32'):
            if paths == []:
    ...
    def getHome(self):
        ...
        if (sys.platform == 'win32'):
            home = self.ros.os_sep.join(['','home','sfscript'])
        else:
    ...
    def get_named_section(self,name):
        if (self._sections.has_key(name)):
            return self._sections[name]
        else:
            return {}
        
    def get_sForce3_section(self):
        d = self.get_named_section('sForce3')
    ...
    def get_main_section(self):
        return self.get_named_section('main')
        
    def get_email_section(self):
        return self.get_named_section('email')
        
    def get_root(self):
        return self.__root__
        
    sForce3 = property(get_sForce3_section)
    main = property(get_main_section)
    email = property(get_email_section)
